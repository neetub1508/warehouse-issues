...
| 4 | **Stock status** | `whb_stock_statuses` | `is_available`, `is_allocatable`, `is_shippable`, `is_countable`, `is_owned_asset`, `requires_reason_to_enter`, `requires_reason_to_leave`, `badge_variant` | **PNR-1** | **Nine.** `AVAILABLE`, `QC_HOLD`, `DAMAGED`, `QUARANTINE`, `EXPIRED`, `BLOCKED`, `CORE_UNGRADED`, plus `PENDING_RESOLUTION` (`RE-005`; `WH-SC-249`'s blocked-move queue holds stock in it) and `RETURNED` (`WH-SC-057` and `DATA-MODEL.md` §`wh_return_receipt_lines` both require a dedicated return status — *"never straight to `AVAILABLE`"* — so it is v1 seed, not a `P2` addition). **Not** `BONDED`: the regime is `duty_status` (`IRR-12`) and a customs hold is a `wh_hold_types` row, so one fact is not recorded on two axes (`RL-001`) |
