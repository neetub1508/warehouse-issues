...
`DECISIONS.md` §6: **`IRR-01`…`IRR-63`** for the consolidated rows, and **`PNR-1`…`PNR-4`** for the four
points of no return.
...
| R7 §6 | `IRR-17` `company_id`/`warehouse_id` · `IRR-25` external-ref tables · `IRR-26` stable string keys · `IRR-28` document/source-system registries · `IRR-29` transit/mobile location types · `IRR-30` `assigned_user_id` · `IRR-31` `whb_item_types` · `IRR-33` `whb_counterparties` · `IRR-37` value-only movements · `IRR-48` `actor_type`/`device_id` · `IRR-49` batch posting · `IRR-63` permission namespace | **12** |
| **Sole-source rows** | | **42** |
...
runs `IRR-01` → `IRR-63` without a break.

---

### Block A — The shape of the ledger (`IRR-01` … `IRR-05`, `IRR-62`)
...
| **IRR-12** | **`duty_status`** on every movement line and in the position key — `DOMESTIC` · `BONDED` · `MOOWR` · `SEZ` · `FTWZ` · `EXPORT_UNDER_BOND` | `whb_stock_movement_lines`, `whb_stock_positions` | base | **PNR-1** | **UB** | **Bonded and duty-paid stock of one SKU must never merge into one balance.** Once a year of movements has commingled them no algorithm separates them, the ex-bond Bill of Entry cannot consume identified bonded quantity, and the warehouse cannot be licensed retrospectively over the mixture. This is a **customs offence, not a data-quality issue** — the only row on this list whose failure mode is criminal rather than commercial. Feature ships v2 (`warehouse-india`); the column ships v1 | R5 row 2, `S-044`, `D-5`, `D-8` |
...
| **IRR-30** | **`whb_locations.assigned_user_id`** | `whb_locations` | base | **PNR-1** | **RK** | The van, the technician's boot and the last-mile driver's stock are locations with an owner-of-custody. Without the column van stock becomes a separate table with a separate reconciliation problem, and `warehouse-adapter-field-service` (v1.1) is unbuildable on the ledger. `field-service` already knows where the van is — `job_trips` + `job_track_points` — and has **no stock table at all** | `[R7 §6 13]`, `T-080`, `C-034` |
...
| **IRR-42** | **`hsn_code` snapshotted** on the movement or document line | `whb_stock_movement_lines.hsn_code` | base | **PNR-1** | **UB** | HSN reclassification is retrospective in effect: reading the item master later gives the **new** code for **old** documents, so the historical GST return no longer reconciles to the system that produced it. One `VARCHAR(8)` on the line. Note that `accessories` has no HSN on its item master at all — the module's only `hsn_code` is on `accessory_quotation_pricing_components` (`accessories/…/V30066__…:37`), a pricing line, not a product | R5 row 18, `S-031`, `E-047` |
...
### Block G — Allocation, tasks and event plumbing (`IRR-45` … `IRR-52`)
...
### Block H — Masters, statutory identity and platform (`IRR-53` … `IRR-61`, `IRR-63`)
...
| **IRR-57** | **`legal_entity_id` + `tax_registration_id` + `state_code`** on the warehouse | `whb_warehouses` | base | **PNR-1** | **UB** | Without them a historical transfer **cannot be classified as supply vs non-supply**, so it cannot be established whether a tax invoice was legally required — for a period whose return has already been filed. This is a filed-return exposure, not a report gap. `D-8` keeps the *rules* out of the core; these three columns are the *hooks*, and they are core | R5 row 19, `S-022`, `D-8` |
...
**Row count check.** `grep -c '^| \*\*I-' docs/IRREVERSIBLE.md` → **63** ids present in the eight
blocks, of which **`IRR-18` is the retired placeholder**, leaving **62 populated rows** against 102
source rows.
...
| `whb_locations` | **PNR-1** (referenced by every line) then **PNR-3** | the migration creating `whb_locations` + `whb_location_types` — **must precede `P0-02`**, because `IRR-05` requires virtual locations to exist before the first movement can balance | `IRR-05` `IRR-16` `IRR-29` `IRR-30` `IRR-44` `IRR-58`; `IRR-26` |
| `whb_warehouses` | **PNR-1** | same migration set as locations | `IRR-17` `IRR-57` |
...
| `whb_outbox` · `whb_outbox_subscriptions` | **PNR-1** for granularity; **PNR-3** for the subscription table | the outbox `CREATE TABLE` | `IRR-50` |
| `whb_stock_periods` | **PNR-1** — `IRR-22` is `NOT NULL` on the movement | the period table's `CREATE TABLE`, **before `P0-02`** | `IRR-22` |
| `whb_stock_position_snapshots` | **PNR-3** — the job's start date | the snapshot table + the scheduled job | `IRR-51` |
...
| `idempotency_key` | VARCHAR(200) | **no** | v1.1 (offline) | `uk(source_system, idempotency_key)`. **Never server-generated** | `IRR-04` |
...
| `reversal_of_movement_id` | UUID FK self | yes | — | If v1 permits `UPDATE`, the corrections are already invisible by the time this arrives | `IRR-02` |
| `reversed_by_movement_id` | UUID FK self | yes | — | Denormalised for *"is this still live"* | `IRR-02` |
...
| `approval_status` · `approved_by` · `approved_at` | — | yes | v1 (scrap), v1.1 (write-down) | For movement types with `requires_approval` | `IRR-27` |
...
| `stock_status_code` | VARCHAR(40) FK → `whb_stock_statuses` | **no** | v1 (QC hold), v1.1 (grading), v2 (bonded) | Otherwise condition and place are permanently conflated | `IRR-10` |
...
| `duty_status` | VARCHAR(30) | **no** | **v2** (`warehouse-india`) | Default `DOMESTIC`. Bonded and duty-paid stock of one SKU must never merge — customs offence, not data quality | `IRR-12` |
...
| `hsn_code` | VARCHAR(8) | yes | **v2** (India) | Reading the item master later gives the *new* code for *old* documents | `IRR-42` |
...
whb_attribute_keys, attribute_value, value_type)`. **Registered keys only. No JSONB** — CLAUDE.md's
...
| `hsn_code` | **v2** | Snapshotted onto the line at post (`IRR-42`) | `IRR-42` |
...
`whb_item_identifiers` — `(item_id, identifier_type, identifier_value, uom_code, pack_quantity,
is_primary)`, `uk` on `(identifier_type, identifier_value)`. **`uom_code` + `pack_quantity` are v1**
...
| `assigned_user_id` | **v1.1** (van stock) | Without it van stock is a separate table and a separate reconciliation problem | `IRR-30` |
...
| `whb_warehouses.legal_entity_id` · `tax_registration_id` · `state_code` | **v2** (India) | Without them a historical transfer cannot be classified as supply vs non-supply, for a filed period | `IRR-57` |
| `whb_warehouses.branch_id` | v1 | **One branch, one GSTIN.** Do **not** repeat accessories' many-to-many junction (`accessory_warehouse_branch`, `V30018:8`) — `branches.branch_type` already admits `'WAREHOUSE'` and `'DISTRIBUTION_CENTER'` (`platform/…/V149:61,63`), so this is free | `C-016` `C-030` |
...
| `whb_lpns` | `owner_id`; **`received_at`** (v2 feature, v1 column — anniversary storage billing has no other anchor); `parent_lpn_id` for nesting; `status` (`OPEN`/`CLOSED`) | `IRR-15` `IRR-06` |
...
| 4 | **Stock status** | `whb_stock_statuses` | `is_available`, `is_allocatable`, `is_shippable`, `is_countable`, `is_owned_asset`, `requires_reason_to_enter`, `requires_reason_to_leave`, `badge_variant` | **PNR-1** | `AVAILABLE`, `QC_HOLD`, `DAMAGED`, `QUARANTINE`, `EXPIRED`, `BLOCKED`, `CORE_UNGRADED`, `BONDED` |
...
| 6 | **Reason code + context** | `whb_reason_codes` | `context` (**no `CHECK`**), `requires_note`, `owning_module`, `blocks_posting`, `itc_treatment`, `statutory_category` | **PNR-1** | adjustment, short-pick, damage, transit-loss, scrap, return, count-variance, hold, release, RTO, NDR |
...
| 12 | **Disposition** (return/RMA outcome) | `whb_dispositions` | `movement_type_code`, `target_stock_status_code`, `requires_inspection`, `emits_credit_signal` | PNR-3 | `RESTOCK_SELLABLE`, `RESTOCK_UNSELLABLE`, `REFURB`, `REPACK`, `SCRAP`, `RTV`, `DONATE`, `HOLD_FOR_CLIENT`, `RETURN_TO_CLIENT` |
