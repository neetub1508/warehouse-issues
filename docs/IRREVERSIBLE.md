...
| 4 | **Stock status** | `whb_stock_statuses` | `is_available`, `is_allocatable`, `is_shippable`, `is_countable`, `is_owned_asset`, `requires_reason_to_enter`, `requires_reason_to_leave`, `badge_variant` | **PNR-1** | `AVAILABLE`, `QC_HOLD`, `DAMAGED`, `QUARANTINE`, `EXPIRED`, `BLOCKED`, `CORE_UNGRADED` — **not** `BONDED`: the regime is `duty_status` (`IRR-12`) and a customs hold is a `wh_hold_types` row, so one fact is not recorded on two axes (`RL-001`) |
...
| 6 | **Reason code + context** | `whb_reason_codes` | `context` (**no `CHECK`**), `requires_note`, `owning_module`, `blocks_posting`, `tax_treatment_code`, `statutory_category` | **PNR-1** | adjustment, short-pick, damage, transit-loss, scrap, return, count-variance, hold, release, RTO, NDR |
