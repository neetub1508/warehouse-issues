...
| warehouse.adjustment.auto_post_below_thresholds | false | boolean; install scope only | App / P2-01 (V510224) | Decision 2026-09-18. Off: an adjustment below both approval thresholds still routes to a second user, whose approval the movement's ledger approval takes. On: it posts on submit, one user, through ADJUST_UP_AUTO / ADJUST_DOWN_AUTO (requires_approval false; any other poster of those types is refused); the negative-stock policy still applies. Off by default, because four eyes is the safe state. |
