...
| `POST` | `/api/warehouse/movements/{id}/reverse` | post the mirror; own idempotency key; mandatory reason; optional `occurred_at`/`posting_date`, current-dated when omitted (`PC-20`); optional `negative_stock_acknowledgement` (§2.4, `MPR-GRD-16`); **no** `period_override` | `warehouse:movements:reverse` | `FR-035` |
...
| `negative_stock_acknowledgement` | object `{ acknowledged` BOOLEAN, `reason_code_id` UUID FK → `whb_reason_codes` `}` | O / conditionally **R** | required to post an outbound line beyond physical on-hand under an effective `WARN` negative-stock policy (`MPR-GRD-16`); `acknowledged` required when sent; a missing `reason_code_id` is the writer's `422 REASON_CODE_REQUIRED`, an inactive or unknown one `422 UNKNOWN_REASON_CODE`, both on `negative_stock_acknowledgement.reason_code_id`; honoured only for a caller holding `whb_negative_stock_policies:override` (else `403 NEGATIVE_STOCK_OVERRIDE_FORBIDDEN`) | **Not a header column** — it gates the post; the breach is recorded in the insufficient-stock log (`MPR-T4-10`). Read on a post that goes straight through (a type without `requires_approval`); a `PENDING` movement's availability is checked at approval with the **approver's** acknowledgement (`MPR-GRD-16`). Part of the received body, so it joins the `PC-17` hash. Ignored when no line breaches a `WARN` policy (CL-1 defect WH-SC-018, 2026-09-17). **Also carried on `/reverse`** (single, and the reprocess (FR-045) of a stored `/reverse` request), same shape and the same checks, for a reversal type that posts straight through (`RECEIPT_REVERSAL`, `TRANSFER_ARRIVE_REVERSAL`, `TRANSFER_RETURN_REVERSAL`, `RETURN_RECEIPT_REVERSAL`, `RTO_RECEIPT_REVERSAL`) and whose outbound mirror breaches a `WARN` policy; it joins the reverse's received-body hash. The reverse carries **no** `period_override`: a reversal into a `SOFT_CLOSED` period stays refused (user decision, WH-SC-022) (review F2, 2026-09-17) |
