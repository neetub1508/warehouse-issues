...
| A12 | The inter-branch transfer stores **two** numbers, with elimination data (`FR-244`) | `AHO-X-06` | **columns BUILT** by `P1-17` (`V510031`, `eec66e88d0`, 2026-09-20): `transfer_price`, `cost_value`, `transfer_price_basis`. P2-18 reads them — `wh_transfer_orders` does not exist; `P1-17` is sequenced after `P2-18` (`AHO-OPEN-06`) |
...
| 2026-09-20 | `AHO-OPEN-08` | **Confirmed as built, no code written.** The queue's named owner is `requestedByName` (`created_by`, the posting actor) and its ageing clock is `ageDays`, both grid AND export columns since `P2-22`'s `V501071`, neither sortable nor filterable. `P2-18` verified the four surfaces (grid component, export service, `default_columns`, `V501071`'s own assertion) and added nothing. The row stays **OPEN** on the wider question `FR-232` asks - whether a requester who may be a floor operator is the person who WORKS the queue - which needs an assign verb (`WS-043`'s `:assign` precedent) and a person's decision | `P2-18` build agent |
| 2026-09-20 | `AHO-OPEN-05` | **Build decision, row stays OPEN.** No source maps movement types to `FR-247`'s five terms, so WS-219 derives the map from the registry rather than from a list in code: `VALUE_ONLY` → revaluations; `reason_context` in (`ADJUSTMENT`, `COUNT_VARIANCE`, `SCRAP`, `TRANSIT_LOSS`, `MASTER_MERGE`) → adjustments; else `IN` → receipts, `OUT` → issues, `INTERNAL` → adjustments. A **reversal type takes the bucket of the type it mirrors**, resolved through `whb_movement_types.reversal_type_code` (`RL-003`), so a reversal nets against its original's bucket. `OPENING_BALANCE` is `IN` and therefore a receipt, which is what makes `AHO-RPT-02` true. An adapter's own movement type is classified by the same two columns with no commit to warehouse (`I-18`). Precedent: the registry's behaviour columns are the module's one classification source (`FR-003`) | `P2-18` build agent |
| 2026-09-20 | `AHO-OPEN-07` | **Build decisions, row stays OPEN.** (a) Grain: the port is asked at company x site x period-end - a control account is keyed by account, not by owner or item group - and the one answer is shown against that site's rows; an implementation that can answer no finer returns empty rather than guessing. (b) `difference` = `closingValue - glControlAccountBalance`, absent whenever the balance is; the `differenceOnly` FILTER is a database-side exception filter (`closingValue <> handedOverValue`, labelled *Not Fully Handed Over*) because the balance is read through a port and cannot be a SQL predicate. (c) `NOT_APPLICABLE` value is in none of the three split columns, and the screen says so - the three need not sum to closing. (d) The balance is read whenever a provider bean exists, independent of `warehouse.accounting.mode`: the mode governs DELIVERY, and this is a read. Precedent throughout: `WhbAccountingPeriodLockProvider` + `WhbPeriodCloseChecker.accountingLock` (`FR-251`) | `P2-18` build agent |
| 2026-09-20 | `AHO-OPEN-10` | **Partly answered in code, row stays OPEN.** WS-219's statistics tiles are rows, closing, handed over, pending and rejected - five, filter-aware, uncached (`FR-395`); the variance is deliberately NOT a tile, because on a Mode-A install it cannot be computed and a 0.00 tile would state a reconciliation nobody performed. Empty state: *"No posted movements match these filters in the selected posting-date window."* `BUILD-SPEC-SCREENS.md` §7 and `PORT-AND-ADAPTER-CONTRACT.md` are NOT amended from the build - that is this row's own work | `P2-18` build agent |
| 2026-09-20 | `AHO-UNR-06` | **Enforcement test written**, closing the row's *"Enforcement test: TBD"*. `WhbAccountingSeamContractTest.GlBalancePort` scans every warehouse module's Java for an `implements` list naming `WhbGlBalanceProvider` and fails the build on one, with a planted-fixture check so it cannot pass vacuously. The port itself is `warehouse-base`'s `WhbGlBalanceProvider`, named to the module prefix like `WhbAccountingPeriodLockProvider` | `P2-18` build agent |
| 2026-09-20 | `AHO-OPEN-12` | **One more drift recorded, row stays OPEN.** `RC-006` calls for an `ownerType` filter defaulting to `OWN`; `classic` seeds no `OWN` owner type - the house type is `HOUSE` (`V500007`, `is_house = true`). WS-219 ships the filter over the fetched registry with no compiled default, so the screen shows every owner type until one is chosen | `P2-18` build agent |
| 2026-09-20 | `AHO-OPEN-07`(a), `AHO-OPEN-07`(b), `AHO-RPT-03`, `AHO-RPT-04` | **Which rows carry a variance — build decision amended, row stays OPEN.** The earlier decision "the one answer is shown against that site's rows" was wrong in arithmetic: the grid's grain is period × company × site × owner × item group, so subtracting the one site balance from EVERY row reports a variance on every part of a site that ties exactly (SPARES 300,000 + CONSUMABLES 200,000 against a GL of 500,000 showed −200,000 and −300,000), and a `ZERO_BAILMENT` owner row, whose closing is 0 by construction, showed −(whole site balance) forever. Amended: `glControlAccountBalance` AND `difference` are emitted **only on a row that IS the whole site** — the answer holds exactly one row for that company × site × period-end, counted by a window over the filtered set so paging cannot change it, AND nothing could have hidden part of that site — no owner, owner-type or item-group filter, no `differenceOnly`, no search, and a caller who may read every owner; otherwise a filtered-down site would look like a site total and reproduce the same bug through the filter strip. On every other row both are absent and `glBalanceReason` states *"The GL control account is kept per site, so a variance is stated only on a site total"*, through the same channel Mode A already uses, so grid cell, view modal and export all explain it with no new plumbing. (b) is unchanged: `difference` is still `closingValue − glControlAccountBalance` wherever it exists, and `differenceOnly` is still the database-side `closingValue <> handedOverValue` exception filter | `P2-18` fix agent, on `functional-reviewer` F1 |
| 2026-09-20 | `AHO-RPT-01`, `AHO-RPT-02` | **What `openingValue` means under a posting-date window.** The posting-date bounds are applied INSIDE the grain CTE and the opening window runs over that CTE, so with `postingDateFrom` set (it is in `default_filters`) `openingValue` is the opening **at the start of that window**, not at the start of the period, and `closingValue` is the window's net. `AHO-RPT-01`'s identity still holds exactly — opening, the five terms and closing are all measured over the same window — and `AHO-RPT-02` still holds for an unwindowed read. But a window figure is not comparable to a control-account balance **as at a date**, which knows nothing of the window, so on any read with either posting-date bound set `glControlAccountBalance` and `difference` are absent and `glBalanceReason` says why. The bounds are deliberately NOT moved outside the window: moving them would change which rows are shown. The screen states it too — the Opening Value column is relabelled *Opening Value (window)* with the reason as its help, and `stockToGl.hints.identity` names it, in en/fr/hi | `P2-18` fix agent, on `functional-reviewer` F2 |
| 2026-09-20 | `AHO-OPEN-10` | **Statistics strip amended, row stays OPEN.** The five tiles are unchanged (rows, closing, handed over, pending, rejected — filter-aware, uncached), but three of them read 0 on every STANDALONE install, because every movement stays `NOT_APPLICABLE` and none of the three buckets ever matches. The grid cells already explained that and the strip did not, so a reader saw *Pending 0* and read "nothing is pending" instead of "not applicable here". The strip now carries the grid's own explanation as one line beneath the tiles (`stockToGl.hints.handover`, en/fr/hi). `BUILD-SPEC-SCREENS.md` §7 and `PORT-AND-ADAPTER-CONTRACT.md` are still NOT amended — that remains this row's own work | `P2-18` fix agent, on `functional-reviewer` F3 |
| 2026-09-20 | `AHO-RPT-06`, `AHO-OPEN-07`(c) | **The full posting-status map is stated, not just the standalone case.** The three split columns map `POSTED` → handed over, `PENDING` **and `BLOCKED`** → pending, `REJECTED` → rejected; `DISCARDED` and `NOT_APPLICABLE` are counted in **none** of the three. The screen hint named only the standalone case, so `BLOCKED` looked unclassified and `DISCARDED` looked lost. Documented, not changed: `stockToGl.hints.handover` now names `BLOCKED` as pending and `DISCARDED` as counted in none, in en/fr/hi, on both the view modal and the statistics strip. No SQL change and no new column | `P2-18` fix agent, on `functional-reviewer` F4 |
| 2026-09-20 | `AHO-OPEN-07`(a) | **The guard list gains `periodId`, row stays OPEN.** The site-row-count partition is `(company_id, warehouse_id, end_date)`, and `whb_stock_periods`' EXCLUDE guard COALESCEs only the all-sites NULL onto the nil UUID — so an all-sites period and a site-specific period of ONE company may share an `end_date`. A `periodId` filter then leaves a row counted as `site_row_count = 1` that is only half the site, and the variance stated on it is the same arithmetic the previous amendment removed. `periodId` therefore joins `ownerId`, `ownerType`, `itemCategoryId`, `differenceOnly`, the search and a scoped owner list in the guard that suppresses the variance; the full list is now **`periodId`, `ownerId`, `ownerType`, `itemCategoryId`, `differenceOnly`, `search`, and a caller who may not read every owner**. Pinned by `WhStockToGlQueryServiceTest.shouldStateNoVarianceWhenThePeriodFilterHalvesTheSite`, and the owner-scope limb by `shouldStateNoVarianceForAnOwnerScopedCaller`. No SQL change | `P2-18` fix agent, on the scoped re-verify (R1) |
| 2026-09-20 | `AHO-OPEN-07`(b) | **Note only — the filter is unchanged.** `differenceOnly` selects on `(opening + net) <> handed_over`, and both sides are measured on the posting date INSIDE the grain CTE, so under a posting-date bound the row **selection** is windowed exactly as the figures are: the filter then answers "not fully handed over within this window", not "within this period". Stated on the screen (`stockToGl.hints.differenceOnlyWindow`, en/fr/hi, shown when both filters are in force) and in the filter component's own doc. No SQL change and no new filter | `P2-18` fix agent, on the scoped re-verify (R2) |
| 2026-09-20 | `AHO-RPT-04`, `AHO-OPEN-07`(a) | **The three suppression reasons are translated, row stays OPEN.** The reasons for the three reads that carry no variance — site grain, partial site, posting window — were English Java constants rendered raw in two grid cells and the modal alert, so an fr/hi reader met a ~150-character English sentence twice per row on a very common read. Each now travels as a reason **code** (`SITE_GRAIN`, `PARTIAL_SITE`, `POSTING_WINDOW`) beside the free text, and the screen renders `warehouse:stockToGl.reasons.<CODE>` through `registryLabel`, the helper every other warehouse screen uses for a code, falling back to the free text so a real `WhbGlBalanceProvider`'s own message — which has no code and no key — still shows as it arrived. The EXPORT is unchanged and still writes the sentence: it has no translator and every header in it is an English literal, so the file reads in one language throughout. `clearCost()` (FR-450) is unchanged — neither the reason nor its code is a figure. Also: the statistics strip no longer reuses the row-level handover note, which read "this row's valued movements" above whole-set tiles (`stockToGl.hints.handoverStrip`, en/fr/hi) | `P2-18` fix agent, on the scoped re-verify (N1, F3) |
| 2026-09-20 | `AHO-RPT-05`, `AHO-X-06` | **Not built, and stated rather than faked.** The `difference` drill has no target: WS-209 Movement Register is `P2-20`'s and does not exist, so the cell opens the details modal, which names the period, site, owner and item group the drill would carry. The transfer's two numbers are header-level on `wh_transfer_orders` (`P1-17`, `V510031`) and the three-leg workflow that would post them is `P2-02`'s, so no WS-219 row can show a transfer leg yet | `P2-18` build agent |
| 2026-09-20 | `AHO-RPT-01`, §5.2 Filters | **A named owner or site outside the caller's scope is REFUSED, not answered empty.** WS-219's three read paths (`getForManagement`, `getStatistics`, `findForExport`) narrowed every read by the caller's owner grants and branch tier but never checked the owner or site the caller NAMED in a filter, so `ownerId=<ungranted owner>` and `warehouseId=<site outside the branch tier>` each returned `200 {total:0}` - indistinguishable from "no stock this period" on a finance reconciliation. Both named values are now guarded the way the ~28 sibling query services in the module guard them: `WhbOwnerScopeResolver.requirePermitted(ownerId)` (403 `OWNER_NOT_PERMITTED`, PC-05) and `WhbWarehouseScopeService.requirePermitted(sites, warehouseId, label)` (403, WH-SC-242), the site check before the no-site short-circuit. An UNFILTERED read for a scoped caller is unchanged - the two-axis narrowing is correct - and so are the suppression reasons, the export columns and V511065. Pinned by `WhStockToGlQueryServiceTest.shouldRefuseANamedOwnerOutsideTheCallersGrants`, `shouldRefuseANamedSiteOutsideTheCallersBranchTier` and `shouldReturnRowsForAPermittedOwnerAndSiteFilter` | `tester`, runtime re-verify (L2-SCOPE-NAMED-FILTER-SILENT-EMPTY) |
...
| AHO-OPEN-06 | **CLOSED 2026-09-20** — `P1-17` built `wh_transfer_orders` with the two numbers (`V510031`) ahead of `P2-18`, on the user's ruling. Residual for `P2-18`: both amounts are **header-level** (DATA-MODEL §2.2 WH-21 gives the line table no value column), so a partially received transfer can be apportioned but not derived per line. Original text: **The transfer's two numbers** (`FR-244`) could not be built: `wh_transfer_orders` does not exist in `classic`, and `P1-17`, which owns the columns, is sequenced after `P2-18` (batch driver ruling on `P2-02`) | p2-18 Scope (*"this task owns the first truth"*) | `IMPLEMENTATION-PLAN.md` §8.2; `P1-17` not built | X-06, A12 | **OPEN** |
