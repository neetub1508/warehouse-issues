...
**144 tasks · 8 phases · 4 versions · 5 modules · 469 requirements · 317 tables · 239 screens.**
...
| **P1** | Masters, identity, inbound | v1 | `warehouse-base` + `warehouse` | 21 | 107 | `V500009`–`V500020`, `V500050`–`V500055`, `V510010`–`V510018`, `V510031`, `V511000`, `V511001`, `V511010`, `V511200` |
...
the D-9 mitigation tables, the activity-history view) ·
...
`V511000` + `V511001` + `V511010` + `V511200` (app permissions, dependencies, menus, admin settings).
**No grid-configuration block:** each grid ships its `grid_column_definitions`, `filter_definitions`
and `grid_preferences` in the migration that creates its own table (settled 2026-09-16, `P1-20`), so
`V501050`–`V501069` and `V511020`–`V511059` are unallocated.
...
**144 tasks.** The count fell out of the decomposition; it was not targeted. Round 5 (2026-09-14) added `P1-22` to correct built work under `D-14` item 8. Round 4 filed six —
...
| **P0-06** | **Owners.** `whb_owner_types` + `whb_owners` with the house owner seeded; `owner_id` on every line, position, lot, serial, LPN, reservation and cost layer; two owners in one movement under `balance_rule`; `is_financial` + `cost_basis` and the never-value-non-own rule; consignment/VMI/customer-owned/job-work as one model in three configurations; owner grants and the **single server-side owner resolver** with a contract test. `whb_owner_companies` (`RG-013`), once a *v2 increment* from `P5-24`, moved to `P1-22` at v1 with `V500073` (`D-14` item 8) | base | `V500007` `V500044` | `WS-009` `WS-019` `WS-020` | `FR-108` `FR-109` `FR-111…FR-114` `FR-406` | P0-04 |
...
### 2.2 · P1 — Masters, identity, inbound · 22 tasks · v1 · `warehouse-base` + `warehouse`
...
| **P1-14** | Quality inspection as a **header over lines** — one inspection number per GRN, plans and criteria, results, and the rollup rules stated to the value: all pass → `PASS`, all fail → `FAIL`, anything mixed → `PARTIAL`. *seed correction* (2026-09-14, `RQP-OPEN-12`): the `REJECTED` stock status and the `REJECT` disposition QC's reject writes. *v2 increment* (from `P5-24`): `wh_inspection_plan_assignments` (`RG-018`) | app+base | `V510015` `V510016` `V500058` `V510220` | `WS-079` `WS-080` | `FR-133` | P1-13 |
...
| **P3-20** | **`warehouse-adapter-field-service`** — the van as a **mobile location owned by a technician**, replenished by a transfer, consumed at the job. It works because the current `CUSTODIAN` row of `whb_location_user_assignments` and the `MOBILE`/`VEHICLE` location types shipped in `P1-05` | ad-fs | `V522000` `V522010` `V522011` `V522012` `V522100`–`V522149` | `WS-203` `WS-204` `WS-205` | `FR-363` | P0-14 P2-02 |
...
| Task | Title | Mod | Migrations | Screens | Closes | Dep |
|---|---|---|---|---|---|---|
| **P5-01** | **`warehouse-3pl`** scaffold, and the proof that it is a module: base and app depend on **neither** it nor its tables — no `whb_`/`wh_` foreign key to a `wh3_` table, no service import, asserted at build time. A client as an **object** (contract dates, notice period, billing cycle and day, currency, terms, credit limit, go-live date, minimum monthly charge and its scope, tax profile — `RF-007`, reconciled with `DATA-MODEL.md` §2.3 per `RF-008`) linked to base by exactly one structural edge, `uk(owner_id)`, **pointing down**; onboarding as a **task set from a template** with a per-client document series; and the confirmation that **one database per customer stands** — a 3PL's clients are an owner dimension, not tenants | 3pl | `V530000` `V530010` `V530011` `V531000`–`V531099` | `WS-155` `WS-156` `WS-157` | `FR-281…FR-283` `FR-303` | P0-06 P2-29 |
| **P5-02** | **Charge codes** as a master (category, default UoM, recurring / pass-through / taxable flags, tax code, SAC/HSN, revenue treatment); **rate cards versioned and effective-dated** (`DRAFT` → `ACTIVE` → `SUPERSEDED`), a card able to inherit a standard card and override lines; every rating modifier a column — free quantity, minimum and maximum charge, rounding, the enumerated basis whitelist and `wh3_storage_aging_bands` (`RF-007`, `RF-008`) | 3pl | `V530020` `V530021` | `WS-158` `WS-159` | `FR-286` `FR-287` | P5-01 |
| **P5-03** | The **billable-event meter — append-only and reversible, exactly like the stock ledger**, with its own idempotency key on `(source_system, source_event_key)`, fed by `P0-11`'s outbox at **billable granularity**. This is why the event vocabulary was fixed in v1. Specified once, in `DATA-MODEL.md` §2.3: the generic subject pair, the rating columns and a `status` ladder in `V530030`, append-only except an allowlist (`RF-008`) | 3pl | `V530030` | `WS-160` | `FR-285` | P5-01 P0-11 |
| **P5-04** | **Storage billing in five methods** — period-end snapshot, period-start snapshot, anniversary (each pallet billed for a storage month from its own receipt date), daily-average and split-month (`RF-007`) — reading `P2-28`'s persisted daily snapshots; and the **minimum monthly charge posting a metered true-up event on a seeded charge code with the arithmetic shown**, not a hidden invoice line | 3pl | `V530031` | `WS-161` | `FR-288` `FR-290` | P5-03 P2-28 |
| **P5-05** | The **billing run as an object with a frozen approved state** — period, type, status ladder, rated/approved/invoiced timestamps, totals; **accessorials** as ad-hoc charges with an author, date, reason and a second-approver threshold; **disputes as records** raisable from the portal, an upheld dispute posting a reversing event; and the AR handover — **`warehouse-3pl` contains no invoice, no numbering sequence and no tax engine, in any version**; `line_no` on the run line as `R-4`'s tie-break (`RF-009`) | 3pl | `V530040` `V530041` `V530042` `V530043` | `WS-162` `WS-163` `WS-164` `WS-170` | `FR-291…FR-294` | P5-02 P5-04 |
...
| **P6-02** | **The best stocking level is computed, not typed**: phase-in after *k* hits in *m* months, a level from the demand window and a days-supply target, phase-out, and the parts manager's override with a reason. This is the single row a parts manager tests us on; and the location-utilisation report, placed at v3 as analysis, never an optimiser (`RC-009`) | app | `V510300` | `WS-243` | `FR-258` `FR-471` | P2-15 P3-18 |
...
|---|---|---|---|
| **1** | `WHB-74` = grid configuration, `V501020`–`V501099`, one migration per grid | Sub-allocated across four tasks: `P0-15` `V501020`–`V501049` · `P2-29` `V501070`–`V501099` · `P3-04` `V501101`–`V501109`. **`P1-20`'s `V501050`–`V501069` released 2026-09-16 (C6 fix 2): unallocated** — every base grid ships its `grid_column_definitions`, `filter_definitions` and `grid_preferences` in the migration that creates its own table, verified across 57 base grids | A single grid-config task spanning four phases cannot be scheduled or reviewed. `V501101`–`V501109` comes out of §7.2's *"reserved for post-v1 base work"*, which is what P3 is |
| **2** | `WH-203` = grid configuration, `V511020`–`V511199` | Sub-allocated: `P2-29` `V511060`–`V511139` · `P3-04` `V511140`–`V511179`. `V511180` is `P2-23`'s `WS-241` grid (row 7, `RK-008`; its v2 increment, from `P5-27`); `V511181`–`V511199` left free. **`P1-20`'s `V511020`–`V511059` released 2026-09-16 (C6 fix 2): unallocated** — each app grid's configuration is its own screen task's acceptance, in that task's table migration (`P1-13`, `P1-14`, `P1-16`) | Same reason. Twenty numbers are left unallocated on purpose, per §7.1 rule 2 |
...
**v1 = P0 + P1 + P2 + P2-IN. 72 tasks, 337 requirements, 168 tables, four modules
...
| Count | Value | Command |
|---|---|---|
| Requirements | **471** | `grep -c '^\| \*\*FR-' docs/WAREHOUSE-FUNCTIONAL-REQUIREMENTS.md` |
| Requirements owned exactly once | **471** | §8.1 |
| Tasks | **144** | `awk 'NF' assign.txt \| wc -l` · `ls issues/p*.md \| wc -l` |
...
| **P1** | 2 | 6 | 10 | 4 | 22 | `P1-05` (facility) and `P1-01`/`P1-02` (item and identity) dominate; the inbound chain is a lot of M |
...
Every one of the 144 tasks, without exception:
