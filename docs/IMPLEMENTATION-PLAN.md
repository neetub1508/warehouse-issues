...
**149 tasks · 8 phases · 4 versions · 5 modules · 469 requirements · 317 tables · 239 screens.**
...
**149 tasks.** The count fell out of the decomposition; it was not targeted. Round 4 added six —
`P3-25` at v1.1 and `P5-24`…`P5-28` at v2 — under `D-14` item 7 (`GAP-REGISTER-R4.md` §4.0). Sizing rule: one task is
...
| **P1-14** | Quality inspection as a **header over lines** — one inspection number per GRN, plans and criteria, results, and the rollup rules stated to the value: all pass → `PASS`, all fail → `FAIL`, anything mixed → `PARTIAL` | app | `V510015` `V510016` | `WS-079` `WS-080` | `FR-133` | P1-13 |
...
| **P2-23** | **Approval permissions distinct from execution permissions**, and the approver may not be the actor — the counter may not approve their own count variance, the adjuster may not approve their own write-off | app | — | — | `FR-408` | P2-01 P2-04 |
...
### 2.5 · P3 — Execution & mobile · 25 tasks · v1.1
...
| **P3-25** | **Simple ABC recompute** (`RK-003`): a monthly job per site over twelve months of `wh_demand_history` issue value, two Pareto cut-offs on the site, the previous class kept for one cycle, a `whb_job_runs` row per run. v1's `WS-093` labels the ABC programme type *manual* until the first run. Velocity and XYZ stay in `P6-02` | base+app | `V500069` | `WS-016` `WS-093` | `FR-463` | P2-15 P2-04 P0-13 |
...
### 2.7 · P5 — 3PL, channels & reverse logistics · 28 tasks · v2
...
| **P5-08** | The **client portal as a permission surface over the existing screens**, not a second application with its own authentication; **owner segregation enforced at the row level in the query layer with a negative test per endpoint**; and the custody-liability and insured-value report for non-own stock — **a different number, on a different report**, never mixed into the inventory asset | 3pl | — | `WS-172` `WS-227` | `FR-115` `FR-284` `FR-300` | P5-01 P0-06 |
...
| **P5-24** | **The v2 association junctions** (`D-14` item 7): UoM defaults per supplier, channel and site (`RG-010`); the site-scoped preferred supplier (`RG-011`); warehouse and owner companies (`RG-012`, `RG-013`); owner-set cages (`RG-014`); outbox-subscription owners, print-template scopes, inspection-plan assignments and a `WAREHOUSE` count-programme scope (`RG-018`); and the tax-scheme side tables (`RG-020`). Every one dated per R22 §1.3 and backfilled from its v1 scalar in the same migration | base+app | `V500070` `V510220` | `WS-007` `WS-016` `WS-017` `WS-019` `WS-023` `WS-029` `WS-056` `WS-079` `WS-093` `WS-131` | `FR-468` | P1-01 P1-03 P1-05 P0-06 P0-11 P1-14 P2-14 P2-04 |
| **P5-25** | **Trade-customer portal** (`RK-006`): a second persona on `FR-284`'s surface, scoped by customer counterparty through the single resolver; availability as a flag, never a quantity; `DRAFT` sales orders for the counter to confirm; shipment status and document download; no pricing engine and no payment. Separate from `P5-08` so the 3PL portal ships alone | app | `V510221` `V511209` `V511239` | `WS-240` | `FR-464` | P5-08 P2-08 P1-08 |
| **P5-26** | **Inter-company movement** (`RK-007`): one action creates company A's sales demand order and company B's purchase order, cross-referenced in the source quad and priced from the transfer-price columns. Never a transfer — `P1-17`'s v1 guard stays | app | — | `WS-090` | `FR-465` | P1-17 P2-08 P1-12 P2-18 |
| **P5-27** | **Value-banded approval levels** (`RK-008`): `wh_approval_levels` — ordered, typed, dated rows per document type × value band × sequence, each naming its approve permission — read by the existing approve actions; `FR-408` at every level and no user approves a document twice. Not a workflow engine | app | `V510222` `V511180` `V511210` `V511240` | `WS-241` | `FR-466` | P2-23 P2-01 P1-12 P2-02 |
| **P5-28** | **Registry row translations** (`RL-015`): `whb_registry_translations` mirroring `menu_translations`, read by `PC-69`'s fallback before the row's own name, on every screen, handheld and printed document | base | `V500071` | — | `FR-469` | P0-04 P1-19 |
...
| **2** | `WH-203` = grid configuration, `V511020`–`V511199` | Sub-allocated: `P1-20` `V511020`–`V511059` · `P2-29` `V511060`–`V511139` · `P3-04` `V511140`–`V511179`. `V511180` is `P5-27`'s `WS-241` grid (row 7, `RK-008`); `V511181`–`V511199` left free | Same reason. Twenty numbers are left unallocated on purpose, per §7.1 rule 2 |
| **2a** | `WH-206` = verb permissions and dependencies, `V511201`–`V511260` | One number from each half per task that ships a transition, claimed in that task's own header; `P2-29` is the gate that checks the claim, not the author. **Claimed in round 4** (`RJ-005`, `GAP-REGISTER-R4.md` §4.0): `P2-01` `V511201`/`V511231` · `P2-02` `V511202`/`V511232` · `P2-04` `V511203`/`V511233` · `P2-10` `V511204`/`V511234` · `P2-12` `V511205`/`V511235` · `P2-13` `V511206`/`V511236` · `P2-15` `V511207`/`V511237` · `P3-11` `V511208`/`V511238` · `P5-25` `V511209`/`V511239` · `P5-27` `V511210`/`V511240` | `V511000` and `V511001` are released; a P2+ task that seeds a verb permission into them would edit an applied migration (`Q-001`) |
...
| **4** | `V510215`–`V510999` = *"reserved for DDL corrections during the app build"* | `V510300`–`V510302` carved out for v3 app DDL (`P6-02`, `P6-03`, `P6-10`); `V510215`–`V510299` left for corrections as intended, less the three round 4 carves (row 7) | §7.3 allocates **no** v3 block for `warehouse`, and three v3 tables need numbers. Carving the top of the reserve is cheaper than opening a new band |
...
| **7** | `V510215`–`V510999` = the app correction reserve; `V511180`–`V511199` = free inside `WH-203`; `V500068`–`V500099` = the post-v1 base gap | Round 4 carves `V510220` (`P5-24`), `V510221` (`P5-25`) and `V510222` (`P5-27`) from the reserve; takes `V511180` for `P5-27`'s `WS-241` grid; takes `V500068` (`P3-06`), `V500069` (`P3-25`), `V500070` (`P5-24`) and `V500071` (`P5-28`) from the base gap; and **releases `V500061`**, whose table moved into `P1-02`'s `V500016` (`RG-008`) — a hole, never reused | `GAP-REGISTER-R4.md` §4.0 allocated every number before the fold, so no fold author picked one (`DATA-MODEL.md` §7.1 rules 1 and 2) |
...
graph TD
  ...
  P325["P3-25<br/>simple ABC recompute"]
...
**`P3-25` (round 4, `RK-003`) hangs off P2, not P3.** Its job reads `P2-15`'s demand history and
...
graph TD
  ...
  P524["P5-24<br/>v2 association junctions"]
  P525["P5-25<br/>trade-customer portal"]
  P526["P5-26<br/>inter-company sale + purchase"]
  P527["P5-27<br/>approval levels"]
  P528["P5-28<br/>registry translations"]
...
**The five round-4 tasks (`D-14` item 7) depend on v1 and hardly at all on each other.** `P5-25` is
the only one with a hard P5 edge: it is a second persona on `P5-08`'s surface. `P5-24`, `P5-26`,
`P5-27` and `P5-28` hang off P0–P2 tasks only (`P1-01`/`P1-03`/`P1-05`, `P1-17`/`P2-08`,
`P2-23`/`P2-01`, `P0-04`/`P1-19`), so they can be scheduled into any gap. The dashed
`P5-24 -.-> P5-18` is `WH-SC-324`: the site-scoped supplier preference is what sister-branch
...
**Round 4 (2026-09-10) moves the expected figures to 469 requirements and 149 tasks.** `FR-460`…`FR-469`
are assigned in §2 to `P1-05`, `P2-25`, `P2-02`, `P3-25`, `P5-25`, `P5-26`, `P5-27`, `P5-09`, `P5-24` and
`P5-28`; `FR-382` moves from `P3-04` to `P0-16`. The result block above is the 2026-09-02 run and is
...
| `FR-070` | `P1·P6` | `P1-03` the class columns | `P3-25` the simple ABC recompute (its own `FR-463`, v1.1, `RK-003`); `P6-02` velocity and XYZ |
...
(`P0-02`), `V500068` (`P3-06`), `V500069` (`P3-25`), `V500070` (`P5-24`) and `V500071` (`P5-28`), and
loses `V500061` (`P3-12`, `RG-008`). `warehouse` gains `V510220`–`V510222` (`P5-24`, `P5-25`, `P5-27`),
`V511180` (`P5-27`) and the ten `WH-206` pairs `V511201`–`V511210` + `V511231`–`V511240` (§2.9 row 2a).
Net **+28**, 823 → **851**: base 150 · `warehouse` 244 · adapters 220 · india 120 · 3PL 117. Tasks
writing no migration stay **32** — `P2-02` now claims its pair and `P5-26` writes none. Re-run the
script above to confirm; the header claims are also what `tools/check-design-set.py --check 4` reads.

**851 numbers, exactly one owner each, every one inside its module's band.** A collision here
...
**32 tasks write no migration.** That is not a defect: they are service-layer work (`P0-03`, `P0-16`,
`P2-16`), frontend/registry work (`P1-18`, `P1-19`), reports over existing tables (`P2-20`, `P2-21`),
or behaviour on a table another task created (`P2-02` on `P1-17`'s transfer orders). **`P6-08` is a
thirty-third of a different kind** — it writes plenty of DDL, and none of it in a band this design set
owns. **One task —
...
| Tasks | **149** | `awk 'NF' assign.txt \| wc -l` · `ls issues/p*.md \| wc -l` |
| Tasks per phase | P0 17 · P1 21 · P2 29 · P2-IN 4 · P3 25 · P4 13 · P5 28 · P6 12 | `awk '{split($1,a,"-"); print (a[1]=="P2"&&a[2]=="IN")?"P2-IN":a[1]}' assign.txt \| sort \| uniq -c` |
| Tasks per version | v1 **71** · v1.1 **25** · v2 **41** · v3 **12** | §9 |
| Requirements per version of owning task | v1 **337** · v1.1 **43** · v2 **73** · v3 **16** — round 4: v1 +4 (`FR-460`…`FR-462`, and `FR-382` in from v1.1), v1.1 +1 −1 (`FR-463` in, `FR-382` out), v2 +6 (`FR-464`…`FR-469`) | §9 |
| Migration numbers allocated | **851** | §8.3 |
...
| **P3** | 1 | 8 | 10 | 6 | 25 | `P3-01` (the RF family, XL) plus a long tail. Mobile is where the estimate is least trustworthy — see §9.6 |
| **P4** | 0 | 4 | 6 | 3 | 13 | Reporting and filing over data that already exists; the hard part is domain correctness, not code |
| **P5** | 1 | 6 | 15 | 6 | 28 | `P5-05` (billing runs, XL — it is money). Two independent sub-streams (3PL; channels/carriers) |
| **P6** | 1 | 3 | 4 | 4 | 12 | `P6-08` (the `logistics` module, XL) is a module, not a task, and will decompose further when it is planned |
| **Total** | **11** | **43** | **64** | **31** | **149** | |
...
**The six round-4 tasks are sized the same way.** `P5-24` (ten junction tables, each with a backfill),
`P5-25` (a second portal persona and its negative tests) and `P5-27` (one table read by four existing
approve actions) are **M**. `P3-25` (one job and four columns), `P5-26` (one action over existing
documents, no DDL) and `P5-28` (one table and one resolver) are **S**.
...
Every one of the 149 tasks, without exception:
