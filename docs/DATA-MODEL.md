...
|---|---|---|---|---|---|
| 1 | Movement type | `whb_movement_types` | `direction` (`IN`/`OUT`/`INTERNAL`/`VALUE_ONLY`), `is_financial`, `cost_basis_default`, `reversal_type_code` (self-FK by code), `requires_approval`, `requires_reason`, `affects_availability`, `is_stock_bearing`, `is_billable_event`, `balance_rule` (`MUST_BALANCE_PER_OWNER_ITEM` / `MUST_BALANCE_PER_ITEM` / `MUST_BALANCE_PER_MOVEMENT`), `is_ownership_transfer` (`I-14`'s flag, default `false`, seeded `true` on `OWNER_CHANGE` and its reversal; added by `V500049`; set on WS-001 like the other behaviour flags, so an install can mark its own type mixed-owner), `reason_context` (`MPR-GRD-11`: the `whb_reason_codes.context` a reason on this type must come from, `NULL` = any; no `CHECK`, no FK; added by `V500084` and seeded where a type has one documented or same-named context, reason-requiring reversals `REVERSAL` (the last five by `V500085`); `ADJUST_DOWN` is `NULL` (`V500085` resets `V500084`'s seed) because `WH-SC-215`'s damage write-off posts as it with a `DAMAGE` reason; a named context must hold an active reason code; set on WS-001) | v1 | **PNR-1** |
...
| Table | Purpose | Key columns | Keys / indexes | FKs | FR | Ver |
|---|---|---|---|---|---|---|
| `whb_reservations` | **An open-item ledger, never a counter.** Every reservation is a row | `company_id`, **`owner_id`**, `item_id`, `location_id`, `lot_id`, `serial_id`, `lpn_id`, `stock_status_code`, `quantity`, `base_quantity`, `uom_code`, **`holder_system`**, **`holder_document_type`**, **`holder_document_id`**, **`holder_line_no`**, `reservation_type` (`SOFT`/`HARD`), `priority`, **`expires_at`** (**`SOFT` rows only** — a `HARD` row ends only by consume, cancel or explicit release, so the expiry job never releases a hold mid-pick, `RJ-007`), `allocation_strategy_id`, `allocation_rule_id` (the **ordering row that decided**, `whb_allocation_strategy_rules` — `OPEN-03`, accepted by `P2-07`), `chosen_reason` (why *this* stock — `FR-173`), `requested_item_id` (`P2-07`, `V500038`: the item the demand asked for, set only when superseding stock was taken — `FR-176`), `released_at`, `release_reason_code_id`, `consumed_by_movement_line_id` | idx(`holder_system`,`holder_document_type`,`holder_document_id`) — the index that makes *"release everything trip X held"* answerable; idx(`item_id`,`location_id`) `WHERE released_at IS NULL`; idx(`expires_at`) `WHERE released_at IS NULL`; idx(`owner_id`,`item_id`) | masters as named; `holder_document_id` is a **generic reference with no FK** (§3.3); `allocation_strategy_id → whb_allocation_strategies`; `consumed_by_movement_line_id` **bare UUID** | `FR-166` `FR-167` `FR-169`–`FR-173` `IRR-45` `L-10` | v1 |
...
| `whb_allocation_rules` | **Which strategy applies**, resolved most-specific-first over the full wildcard tuple — mirrors `whb_gl_posting_rules` rather than inventing a second resolution shape. Withdraws `whb_allocation_strategies.scope_type`/`scope_ref` (`Z-001`). **Copy-on-write once referenced** (`I-24`) | `warehouse_id`, `item_id`, `item_category_id`, `owner_id`, `counterparty_id`, `demand_type_code` (`DEMAND_TYPE`) + `demand_type_code_list` (all nullable = any), `strategy_id`, `specificity` (computed), `allow_supersession` (`P2-07`, `V500038`, default `false`: superseding stock may be taken only under this flag — `FR-176`), `is_active`, **`version_no`**, **`supersedes_id`** | idx(`specificity` DESC, `warehouse_id`, `item_id`) | `(demand_type_code_list, demand_type_code) → whb_code_list_values`; self (`supersedes_id`) | `FR-172` `FR-173` `IRR-47` | v1 |
...
| `whb_accounting_handovers` | One envelope per posting-relevant event, plus the **rejected-handover queue**. **`REJECTED → VOIDED`** via `whb_accounting_handovers:void`, approver ≠ requester, only when the movement is reversed or reclassified `NOT_APPLICABLE`; reversing a never-posted movement voids both envelopes, and `VOIDED` is outside the period-close guard (`RJ-011`) | `movement_id` (**bare**), `occurred_at`, `company_id`, `envelope_kind`, **`envelope_version`** (`SMALLINT NOT NULL`, so a stored v1 envelope replays as v1, `RL-002`), `idempotency_key`, `payload` **`TEXT`**, `payload_hash`, `status` (`PENDING`/`SENT`/`POSTED`/`REJECTED`/**`VOIDED`**), `attempt_count`, `last_attempt_at`, `rejection_code`, `rejection_message`, `external_document_ref`, `alert_raised_at` (the queue alert's episode stamp on `REJECTED` rows - `P2-22`, `V501071`; `VOIDED` leaves the count) | uk(`idempotency_key`); idx(`status`,`last_attempt_at`) `WHERE status IN ('PENDING','REJECTED')`; idx(`movement_id`); idx(`alert_raised_at`) `WHERE alert_raised_at IS NOT NULL` (`V501071`; serves the every-tick status-agnostic clear and the active-episode check, the `REJECTED` count and stamp use idx(`status`)) | `company_id → whb_companies` | `FR-232` `FR-233` `FR-248` `IRR-41` | v1 |
...
| `wh_reconciliation_exceptions` | Ledger-vs-position drift, position-vs-allocation drift and orphaned reservations, **with an owner and an ageing clock** | `exception_type`, `warehouse_id`, `subject_key_text`, `detected_at`, `owner_user_id`, `status`, `resolved_at`, `resolution_note`, `drift_finding_id` (**no `age_days` — the age is computed on read off the injected `Clock`, `RJ-012`; the `ageOverDays` filter becomes a `detected_at < :cutoff` bound so it rides the index**) | idx(`status`,`detected_at`); idx(`owner_user_id`) | `drift_finding_id ↓base whb_position_drift_findings` | `FR-163` | v1 |
...
| `wh_pick_tasks` | The 1:1 pick extension of `whb_tasks`. **`D-3`'s worked example, and the fix for §3.4 defect X-4** | **`task_id`** (uk), `demand_order_line_id`, `reservation_id`, `staged_reservation_id`, `wave_id`, `pick_method` (`DISCRETE`/`BATCH`/`CLUSTER`/`ZONE`), `requested_quantity`, `picked_quantity`, `source_location_id`, `item_id`, `lot_id`, `serial_id`, `lpn_id`, `owner_id`, `pick_sequence`, `staging_location_id`, `is_short_pick`, `short_pick_quantity`, `short_pick_action`, `pick_movement_id` (bare). *P2-09 amendment:* `staged_reservation_id` is the hold re-attached at the staging location (RJ-015) - the row the ship confirm closes; `short_pick_reason_code_id` is dropped because the short-pick reason is the task's own `whb_tasks.exception_code` (a TASK_EXCEPTION code), one fact in one place | uk(`task_id`); idx(`demand_order_line_id`); idx(`wave_id`,`pick_sequence`) | `task_id ↓base whb_tasks`; `reservation_id ↓base whb_reservations`; the rest as named | `FR-185` `FR-186` `FR-188` `FR-212` | v1 |
...
| `wh3_client_onboarding_templates` | **Onboarding is a task set from a template**, so every client is onboarded the same way. Retired, never deleted: retiring keeps the lines, so every client onboarded from it keeps its lineage | `code`, `name`, `is_default`, `is_active`, `status` (`ACTIVE`/`INACTIVE`, moving with `is_active`) | uk(`code`); uk(`is_default`) `WHERE is_default AND is_active` — one ACTIVE default | — | `FR-283` | v2 |
| `wh3_client_onboarding_template_tasks` | | `template_id`, `sequence` (> 0), `task_name`, `owner_role`, `due_offset_days` (≥ 0), `is_blocking` | uk(`template_id`,`sequence`) `DEFERRABLE INITIALLY DEFERRED` | `template_id → wh3_client_onboarding_templates` (`ON DELETE CASCADE`) | `FR-283` | v2 |
| `wh3_client_onboarding_tasks` | The instantiated task set — each template line COPIED at Onboard, so a later template edit never rewrites a client's checklist | `client_id`, `template_task_id`, `sequence`, `task_name`, `owner_role`, `is_blocking`, `assigned_to`, `due_at`, `completed_at`, `completed_by`, `status` (`OPEN`/`COMPLETED`), `evidence_note`; `overdue_alert_sent_for` (`V531002`, the `FR-165` overdue alarm's stamp) | uk(`client_id`,`sequence`); idx(`client_id`,`status`); idx(`assigned_to`); idx(`due_at`); CHECK completed ⇔ `completed_at` and `completed_by` set | `client_id → wh3_clients`; `template_task_id → wh3_client_onboarding_template_tasks` (`ON DELETE SET NULL`); `assigned_to`, `completed_by → users` | `FR-283` | v2 |
...
| **W4** | `wh_pick_tasks.reservation_id` · `wh_pick_tasks.staged_reservation_id` | `whb_reservations(id)` | The pick-face hold, and the hold re-attached at staging (P2-09) |
...
| `P2-07` | `V500038` | `whb_allocation_rules.allow_supersession` (`FR-176`), `whb_reservations.requested_item_id` (frozen with the row by the guard trigger), the `RESERVATION_RELEASE` reason `YIELDED` (`WH-SC-189`) and the `FIXED_LOCATION_FIRST` strategy (`OPEN-01`) | v1 |
| `P2-15` | `V500039` | `whb_number_series_issued.issued_by` nullable, `issued_by_actor_type VARCHAR(20) NOT NULL DEFAULT 'USER'` and the CHECK `(issued_by IS NOT NULL OR issued_by_actor_type = 'SCHEDULED_JOB')` — a number a scheduled job takes names no person (`FR-024`); the 6-argument `whb_next_document_number()` carries the actor type and the 5-argument form delegates with `USER`. Sorts after `V500020` (the table). Claimed from the declared gap, which is now closed. Allocated 2026-09-18 | v1 |
...
| `P0-02` | `V500086` | `whb_stock_period_override_requests` + **forward-only correction of `V500032`/`V500049`**: `whb_assert_period_open()` admits a `PENDING` insert with its override request and checks the approval on `UPDATE OF approval_status` → `APPROVED` (`I-10`, `MPR-GRD-10`, `MPR-T4-06`; CL-1 defect WH-SC-022, user decision 2026-09-17). **Edited in place once** (fix pass 2): its verification block compared the zero-based `tgattr::int2[]` with a one-based array, always raised, and so rolled the migration back on every install — never applied anywhere, so the edit is safe (§7.1 rule 6's exception: no history row can exist) | v1 |
...
| `P2-07` | `V501070` | **WS-048 Allocation strategies.** `whb_allocation_strategies` permissions (view/create/edit/export - no `:delete`, no branch/all tier: a strategy carries no site and no owner) with `permission_dependencies`, the menu leaf under Execution at route `/dashboard/warehouse/masters/allocation-strategies` (`menu_translations` en + fr + hi), and the grid (`grid_column_definitions`, `filter_definitions`: `owningModule`, `orderingKey`, `isActive`; `grid_preferences` with both `default_columns` and `default_filters`). Grants: ADMIN / ADMIN_GROUP every verb, AUDITOR `:view`. The bundle grants and the stale-`HARD` setting are **not** here - see `V501072` | v1 |
| `P2-22` | `V501071` | **WS-052 owner and age, and the handover queue alert.** Grid columns `requestedByName` and `ageDays` (computed on read, `RJ-012`; neither sortable nor filterable) with `default_columns` rewritten whole; `admin_settings` `warehouse.accounting.handover.rejected_queue_threshold` (10, 1–10000); and `ALTER TABLE whb_accounting_handovers ADD alert_raised_at` + partial index `WHERE alert_raised_at IS NOT NULL`, and the same partial index on `whb_inbound_messages` (the every-tick clear on both alert tables is status-agnostic). The column was first allocated `V500039`, which sorts **before** `V500042` (the table) and so cannot hold the `ALTER` on a from-scratch migrate; `V500039` stayed in the gap (claimed later by `P2-15`). Inside the `V501070`–`V501099` grid band (carved out of `P2-29`'s header), an out-of-order insert below `V501100` on an upgraded install. Allocated 2026-09-18 | v1 |
| `P2-07` | `V501072` | **WS-048 bundle grants and the stale-`HARD` threshold - `V501070`'s forward delta.** `V501070` was applied (2026-09-18) before its review fixes; an applied migration is never edited, so they land here. The six internal bundles `whb_allocation_strategies:view` and Finance Controller `:export` (what `V501002` §3 gives every earlier base master; the `V500055` shape), with the bundle menu reachability of `V501010` §3. And `admin_settings` `warehouse.reservation.hard_stale_after_minutes` (1440, 60–43200) - the reservation ageing report's stale-`HARD` threshold, counted from `created_at` (fix cycle F4). Idempotent, forward-only | v1 |
...
| P1-13 | `V510026` | `wh_goods_receipt_lines.order_conversion_factor_used` — PO-unit factor frozen at post (CL-2 F1, 2026-09-17) | `warehouse` |
| — | `V510027`–`V510029` | *gap* | — |
...
| `P2-23` | `V511062` | DDL, no seed: `alert_raised_at` on `wh_stock_adjustments` and `wh_counts` - the `WHB_PENDING_APPROVAL_ALERT` episode stamp (one alert per pending episode, measured from `submitted_at`; `CONFIG-CASE-14`) and a partial index on each table's pending rows. Claimed from the warehouse grid band; `P2-29` now starts at `V511064` | v1 |
| `P2-23` | `V511063` | DDL, no seed: `alert_raised_at` on `wh_receipt_reversals` (episode from `created_at` - created REQUESTED, no `requested_at`) and `wh_purchase_orders` (from `submitted_at` - no `submitted_by`), plus a partial index on each table's pending rows. Round 2 of `CONFIG-CASE-14`; `V511062` is committed and not edited. Claimed from the warehouse grid band; `P2-29` now starts at `V511064` | v1 |
| `P2-09` | `V511064` | Seed row, no DDL: the `REVERSAL` reason `DESTAGED` that `V510041` seeds - an early, uncommitted draft of `V510041` was applied locally without it, and `V510041` is frozen, so the committed end state is forward-seeded, guarded so it is a no-op wherever `V510041` ran in full. Claimed from the warehouse grid band `V511060`-`V511139` (`P2-29` now starts at `V511066`), 2026-09-18 | v1 |
| `P2-18` | `V511065` | Seed rows, no DDL: the `WS-219` grid `wh_rpt_stock_to_gl` (BUILD-SPEC §7's thirteen columns plus `RC-002`'s `ownerName` / `itemCategoryName` / `glControlAccountBalance` and `RC-006`'s `ownerTypeCode` — seventeen — and nine filters), `wh_rpt_stock_to_gl:view` / `:view:all` / `:view:branch` / `:export` with finance-desk grants only, and the menu leaf under `warehouse_periods_accounting`. WS-219 owns no table (§7 note 4). Claimed from the warehouse grid band `V511060`-`V511139` on 2026-09-19 (`AHO-OPEN-09`); `P2-29` now starts at `V511066` | v1 |
