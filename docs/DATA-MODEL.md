...
| `P1-14` | `V510025` | Seeds the `QCINSP` quality-inspection document type into `whb_document_types` (and any other document-type code the C8 inbound tasks post under that was never registered), so dispositions pass the writer's `requireDocumentType` guard. Follows `V500055`'s precedent of a task seeding its own document type. Allocated 2026-09-16 by the C8 runtime-test fix cycle | v1 |
| — | `V510026`–`V510029` | *gap* | — |
