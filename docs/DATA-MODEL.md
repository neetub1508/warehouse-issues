...
| `whb_warehouses` | The site. One building, one yard, or a non-building stock pool. **It is linked to platform branches and never mirrored as one** (`RH-003`): which branches it serves, and the one branch it is registered under, are `whb_warehouse_branches` rows. It carries **no `branch_id`, `tax_registration_id` or `legal_entity_id`** (`D-14` item 2) — the GSTIN, the branch-scoped statutory series and the tax attribution are read through the `REGISTERED` link | `code`, `name`, `company_id`, `warehouse_type`, **`is_physical`**, `timezone` (**validated against platform `timezones`**, `RH-011`), `gln`, **`state_code`** (the site's own address fact, not derived from a GSTIN), address block (line1/line2/city/state/postal/country/`latitude`/`longitude`), `order_cutoff_time`, `default_putaway_strategy_code`, `has_picking`, **`abc_a_cutoff_pct`**, **`abc_b_cutoff_pct`** (`DECIMAL(9,6)`, the two Pareto cut-offs, **v1.1**, `V500069`, `P3-25`) | uk(`company_id`,`code`) — the only key (`RL-004`) | `company_id → whb_companies` | `FR-079` `FR-080` `FR-081` `FR-460` `FR-463` `IRR-17` `IRR-57` `IRR-44` | v1 |
...
| `wh_carriers` | Carrier master. **One of the five relocatable objects** (R7 §2.6) — referenced by stable code, never by an FK from a relocatable table | `code`, `name`, `counterparty_id`, `carrier_type`, `tracking_url_template`, `api_enabled`, `is_active` | uk(`code`); **uk(`counterparty_id`)** — one carrier per party, a 1:1 extension (`RG-016`) | `counterparty_id ↓base whb_counterparties` | `FR-196` `FR-199` | v1 |
...
| Table | Purpose | Key columns | Keys / indexes | FKs | FR | Ver |
|---|---|---|---|---|---|---|
| `wh3_clients` | **A client is an object, not a customer record.** Its parties are `wh3_client_counterparties` (`RG-017`) | `client_code`, `name`, **`owner_id`**, `contract_start_date`, `contract_end_date`, `notice_period_days`, `billing_cycle` (`MONTHLY`/`FORTNIGHTLY`/`WEEKLY`), `billing_day`, `currency_code`, `payment_terms_days`, `credit_limit`, `status` | uk(`client_code`); uk(`owner_id`) — one client per owner | `owner_id ↓base whb_owners` | `FR-282` `FR-303` | v2 |
...
| `P3-25` | `V500069` | `abc_a_cutoff_pct`, `abc_b_cutoff_pct` on `whb_warehouses`; `previous_abc_class`, `abc_computed_at` on `whb_item_site_settings` (`RK-003`). Creates no table | v1.1 |
| `P5-24` | `V500070` | The eight v2 base junctions and side tables: `whb_warehouse_companies`, `whb_owner_companies`, `whb_location_owner_dedications`, `whb_item_uom_defaults`, `whb_item_tax_classifications`, `whb_uom_scheme_codes`, `whb_reason_code_tax_treatments`, `whb_outbox_subscription_owners`; plus `warehouse_id` on `whb_item_supplier_sources` (`RG-010`–`RG-014`, `RG-018`, `RG-020`) | v2 |
| `P5-28` | `V500071` | `whb_registry_translations` (`RL-015`) | v2 |
| — | `V500072`–`V500099` | *gap* — post-v1 base DDL. (`V500067` is `WHB-69`'s, below; the earlier gap row starting at `V500067` was stale) | — |
...
**Round-4 rows carry their owning task id, not a `WHB-`/`WH-` label** (`V500037`, `V500068`–`V500071`
here; `V510220`–`V510222` and `V511180` in §7.3). A label is an id, and `GAP-REGISTER-R4.md` §4.0
...
| `P5-24` | `V510220` | `wh_print_template_scopes`, `wh_inspection_plan_assignments` (`RG-018`) — carved from the correction reserve | v2 |
| `P5-25` | `V510221` | `wh_trade_portal_users` (dated) | v2 |
| `P5-27` | `V510222` | `wh_approval_levels` | v2 |
| — | `V510223`–`V510999` | **Reserved: 777 numbers** for DDL corrections during the app build | — |
...
| `P5-27` | `V511180` | The `WS-241` grid configuration — one grid migration inside `WH-203`'s range, left free by `P3-04` | v2 |
...
| W3-04 | `V530021` | `wh3_rate_cards`, `wh3_rate_card_lines`, **`wh3_rate_card_clients`** (one `ACTIVE` card per client as an `EXCLUDE`, `RG-017`) |
...
7 app, 2 India, 3 3PL; 317 → 358):

| Module | Prefix | Tables | Band | Ships |
|---|---|---:|---|---|
| `warehouse-base` | `whb_` | **129** | V500000–V509999 | v1 · v1.1 · v2 · v3 |
| `warehouse` | `wh_` | **128** | V510000–V519999 | v1 · v1.1 · v2 |
| `warehouse-india` | `whin_` | **58** | V540000–V549999 | v1 wave · v2 wave |
| `warehouse-3pl` | `wh3_` | **24** | V530000–V539999 | v2 · v3 |
...
Re-run 2026-09-10 against this file:

| Version | Base | App | India | 3PL | Adapters | Total |
|---|---:|---:|---:|---:|---:|---:|
| **v1** — the stock ledger and inventory control | 98 | 68 | 16 | 0 | 9 | **191** |
| **v1.1** — execution and mobile | 12 | 22 | 0 | 0 | 8 | **42** |
| **v2** — India statutory, 3PL, channels, reverse logistics | 16 | 38 | 42 | 23 | 2 | **121** |
| **v3** — optimisation, planning, the logistics seam | 3 | 0 | 0 | 1 | 0 | **4** |
| **Total** | **129** | **128** | **58** | **24** | **19** | **358** |

**191 tables in v1 is the number to argue with, and it is deliberately large.** Roughly a third of
